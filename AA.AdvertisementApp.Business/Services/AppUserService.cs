    ...
    public class AppUserService : Service<AppUserCreateDto, AppUserUpdateDto, AppUserListDto, AppUser>, IAppUserService
    {
        ...
        public async Task<IResponse<AppUserCreateDto>> CreateWithRoleAsync(AppUserCreateDto dto, int roleId)
        {
            ...
            if (validationResult.IsValid)
            {
                ...
                await _uow.GetRespository<AppUser>().CreateAsync(user);
                await _uow.GetRespository<AppUserRole>().CreateAsync(new AppUserRole
                {
                    AppUser = user,
                    AppRoleId = roleId
                });
                ...
                return new Response<AppUserCreateDto>(ResponseType.Successs, dto);
            }
        ...
        public async Task<IResponse<AppUserListDto>> CheckUserAsync(AppUserLoginDto dto)
        {
            ...
            if (validationResult.IsValid)
            {
                var user = await _uow.GetRespository<AppUser>().GetByFilterAsync(x => x.Username == dto.Username && x.Password == dto.Password);
                if (user != null)
                {
                    var appUserDto = _mapper.Map<AppUserListDto>(user);
                    return new Response<AppUserListDto>(ResponseType.Successs, appUserDto);
                }
        ...
        {
            var roles = await _uow.GetRespository<AppRole>().GetAllAsync(x => x.AppUserRoles.Any(x => x.AppUserId == userId));
            if (roles == null)
            {
            ...
            return new Response<List<AppRoleListDto>>(ResponseType.Successs, dto);
        }
    }
}
